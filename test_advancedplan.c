...
static void test_IsNumEvenTemplate(int num, int IsFalse)
{
...
static void test_IsThreePointsInALineTemplate(int x1, int y1, int x2, int y2, int x3, int y3, int IsFalse)
{
  ck_assert_int_eq(IsThreePointInALine(x1, y1, x2, y2, x3, y3), !IsFalse);
}
...
static void test_GetGpsLonFromDistanceTemplate(int xRealLength, int xTopLeft, int yTopLeft, int lon)
{
  double lonCal;

  lonCal = (double)GetGpsLonFromDistance(xRealLength, xTopLeft, yTopLeft);
  ck_assert_msg(IsDoubleEqualWithTolerance(lonCal, (double)lon, pow(10, 4)), "xReallength %d  xTopleft %d yTopLeft %d lat %d", xRealLength, xTopLeft, yTopLeft, lon);
}
...
static void test_GetGpsLatFromDistanceTemplate(int yRealLength, int yTopLeft, int lat)
{
  double latCal;

  latCal = (double)GetGpsLatFromDistance(yRealLength, yTopLeft);
  ck_assert_msg(IsDoubleEqualWithTolerance(latCal, (double)lat, pow(10, 4)), "yReallength %d yTopLeft %d lat %d", yRealLength, yTopLeft, lat);
}
